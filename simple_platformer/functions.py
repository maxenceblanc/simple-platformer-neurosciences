...
def loadChunk(block_list, chunk, x_start):
    ...
    for column in range(len(chunk[0])):
        ...
        y = (
            (cfg.VISIBILITY_Y - 1) * levels.CHUNK_HEIGHT * cfg.BLOCK_HEIGHT
        )  # Get back up
    ...
    for block in blocks:
        for pixel in range(-(cfg.BLOCK_WIDTH - 1), cfg.BLOCK_WIDTH):
            if (
                ...
                and player.rect.left == block.rect.left + pixel
            ):
                return True
    return False
...
def score_func(time, player, blocks):
    ...
    speed_weight = 1
    score = (
        chunks_passed * pos_weight + ((chunks_passed * 20) / time**1.5) * speed_weight
    )
