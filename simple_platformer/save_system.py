...
def save(filename, player_name, score, count, secs, chunk_times, nb_chunks):
    ...
    try:
        ...
        data = pd.DataFrame(
            columns=["name", "attempt_n", "score", "count", "time"]
            + ["chunk_" + str(i) for i in range(1, nb_chunks + 1)]
        )

    attempt_n = (
        data[data.name == player_name].shape[0] + 1
    )  # TODO: change to find highest attempt digit?
    ...
    new_entry.update(
        {
            "chunk_" + str(i): format(round(chunk_times[i - 1], 3), ".3f")
            for i in range(1, len(chunk_times) + 1)
        }
    )
    ...
    if len(chunk_times) < nb_chunks:
        new_entry.update(
            {
                "chunk_" + str(i): None
                for i in range(len(chunk_times) + 1, nb_chunks + 1)
            }
        )
